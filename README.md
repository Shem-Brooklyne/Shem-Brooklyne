# Shem Mwangi

### Backend Software Engineer | Java & Spring Boot Specialist

Professional with 4+ years of experience architecting and delivering high-performance microservices and fintech platforms across Africa. Focused on scalability, reliability, and clean code.

---

### 🛠️ Core Tech Stack

- **Backend:** Java, Spring Boot, PHP, Microservices
- **Cloud & DevOps:** AWS, GCP, Docker, Kubernetes, CI/CD
- **Data:** SQL, NoSQL, Firebase
- **Mobile:** Android (Java/Kotlin), Flutter

---

### 📬 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?logo=linkedin)](https://www.linkedin.com/in/shem-mwangi/)
[![GitHub](https://img.shields.io/badge/GitHub-Follow-333?logo=github)](https://github.com/ShemBrooklyne)
[![Portfolio](https://img.shields.io/badge/Portfolio-Visit-orange?logo=firefox)](https://shem-brooklyne.github.io/Shem-Brooklyne/)

**Email:** [shembrooklyne@gmail.com](mailto:shembrooklyne@gmail.com)

---
*Building the future of fintech, one microservice at a time.*
